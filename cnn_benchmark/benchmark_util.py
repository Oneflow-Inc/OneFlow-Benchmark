...
class CNNSpeedometer:
    ...
    def speedometer_cb(self, step, total_batch_size, warmup_num, iter_num, loss_print_every_n_iter):
        ...
        def callback(train_loss):
            if step < warmup_num:
                ...
                if (train_step + 1) % loss_print_every_n_iter == 0:
                    ...
                    duration = self.watch.split()
                    sentences_per_sec = total_batch_size * loss_print_every_n_iter / duration
                    print("iter {}, total_loss: {:.3f}, mlm_loss: {:.3f}, nsp_loss: {:.3f}, speed: {:.3f}(sec/batch), {:.3f}(sentences/sec)"
                        .format(train_step, total_loss, mlm_loss, nsp_loss, duration, sentences_per_sec))
                ...
                if (train_step + 1) == iter_num:
                    ...
                    print("-".ljust(66, '-'))
                    print(
                        "average speed: {:.3f}(sentences/sec)".format(avg_sentences_per_sec))
                    print("-".ljust(66, '-'))
        return callback