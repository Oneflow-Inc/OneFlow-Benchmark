...
parser.add_argument("--weight_l2", type=float, default=0.01,
                    help="weight l2 decay parameter")
...
parser.add_argument("--warmup_iter_num", type=int,
                    default=10, help="total iterations to run")
parser.add_argument("--log_every_n_iter", type=int,
                    default=1, help="print loss every n iteration")
...
def BertDecoder(data_dir, batch_size, data_part_num, seq_length, max_predictions_per_seq):
    ...
    blob_confs.append(_blob_conf('masked_lm_weights', [
                      max_predictions_per_seq], flow.float))
