...
def add_ofrecord_args(parser):
    ...
    parser.add_argument(
        ...
        default=256,
        required=False,
        help="resize shorter for validation",
    )
    parser.add_argument(
        "--train_data_dir", type=str, default=None, help="train dataset directory"
    )
    parser.add_argument(
        "--train_data_part_num", type=int, default=256, help="train data part num"
    )
    parser.add_argument(
        "--val_data_dir", type=str, default=None, help="val dataset directory"
    )
    parser.add_argument(
        "--val_data_part_num", type=int, default=256, help="val data part num"
    )
    return parser
...
def load_imagenet_for_training(args):
    ...
    if args.gpu_image_decoder:
        ...
        image = flow.data.OFRecordImageDecoderRandomCrop(
            ofrecord, "encoded", color_space=color_space  # seed=seed,
        )
