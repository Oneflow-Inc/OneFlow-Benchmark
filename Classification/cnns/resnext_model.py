...
def conv3x3(in_tensor, filters, strides=1, groups=1, trainable=True, name=""):
    return _conv2d(
        ...
        kernel_size=3,
        strides=strides,
        padding=[[0, 0], [0, 0], [strides, strides], [strides, strides]],
        groups=groups,
        use_bias=False,
        trainable=trainable,
        name=name,
...
def basic_block(
    ...
    residual = inputs
    conv1 = conv3x3(
        inputs, filters * 2, strides, trainable=trainable, name=layer_block + "conv1"
    )
    bn1 = _batch_norm(
        conv1, trainable=trainable, training=training, name=layer_block + "bn1"
    )
    ...
    bn2 = _batch_norm(
        conv2, trainable=trainable, training=training, name=layer_block + "bn2"
    )
    if downsample is True:
        residual = _conv2d(
            inputs,
            filters * basic_block_expansion,
            kernel_size=1,
            strides=strides,
            use_bias=False,
            trainable=trainable,
        ...
        residual = _batch_norm(
            residual, trainable, training=training, name=layer_block + "downsampe-1"
        )
...
class ResNeXt:
    ...
    def build_network(self):
        ...
        bn1 = _batch_norm(
            conv1, trainable=self.trainable, training=self.training, name="bn1"
        )

        relu = flow.nn.relu(bn1, name="relu1")
        max_pool = flow.nn.max_pool2d(
            relu,
            ksize=3,
            strides=2,
            padding=[[0, 0], [0, 0], [1, 1], [1, 1]],
        ...
        layer1 = self._make_layer(
            max_pool, 64, self.layers[0], self.num_group, layer_num="layer1"
        )
...
def resnext18(images, trainable=True, training=True, channel_last=False, **kwargs):
