...
from dali import build_input_pipeline
...
def InferenceNet(images: oft.Numpy.Placeholder((32, 224, 224, 4)),
                 labels: oft.Numpy.Placeholder((32,), dtype=flow.int32)):
    logits = model_dict[args.model](images, args)
    predictions = flow.nn.softmax(logits)
...
def main():
    ...
    train_iter, inference_iter = [iter(x) for x in build_input_pipeline()]
    for epoch in range(args.num_epochs):
        ...
        for i, train_batch in enumerate(train_iter):
            TrainNet(train_batch[0].data[0].asnumpy().astype(np.float32),
                     train_batch[0].label[0].asnumpy().astype(np.int32)).async_get(metric.metric_cb(epoch, i))

        # inference
        if args.val_data_dir:
            metric = Metric(desc='validation', calculate_batches=num_val_steps,
                            batch_size=val_batch_size)
            inference_iter.reset()
            for i, inference_batch in enumerate(inference_iter):
                InferenceNet(inference_batch[0].data[0].asnumpy().astype(np.float32),
                             inference_batch[0].label[0].asnumpy().astype(np.int32)).async_get(
                    metric.metric_cb(epoch, i))
