...
train_batch_size = 1
train_record_reader = flow.nn.OfrecordReader("/home/ldpe2g/oneFlow/oneflowBenckmark/dataset/imagenette/ofrecord_224/train",
...
flip = CoinFlip(batch_size=train_batch_size)
...
for i in range(10):
    ...
    # image_raw_buffer = flow.Tensor(image_raw_buffer.numpy(), dtype=flow.uint8, data_initializer=, device=oneflow_api.device('cuda'))
    # rng = flow.Tensor(rng.numpy(), dtype=flow.int8, device=oneflow_api.device('cuda'))
