...
def _TransformerModel(input_blob,
    ...
    for layer_idx in range(num_hidden_layers//compress_ratio):
        prev_output_blob = addOnelayer(layer_idx, True, prev_output_blob, attention_mask_blob, num_attention_heads, attention_head_size, attention_probs_dropout_prob,
                                       initializer_range, seq_length, hidden_size, hidden_dropout_prob, intermediate_act_fn, intermediate_size, all_layer_output_blobs)

    output_shape = (-1, seq_length, hidden_size)
    if do_return_all_layers:
        final_output_blobs = []
        for layer_output_blob in all_layer_output_blobs:
            final_output_blob = flow.reshape(layer_output_blob, output_shape)
            final_output_blobs.append(final_output_blob)
        return final_output_blobs
    else:
        final_output_blob = flow.reshape(prev_output_blob, output_shape)
...
def addOnelayer(layer_idx, trainable, prev_output_blob, attention_mask_blob, num_attention_heads, attention_head_size, attention_probs_dropout_prob, initializer_range, seq_length, hidden_size, hidden_dropout_prob, intermediate_act_fn, intermediate_size, all_layer_output_blobs):
    with flow.scope.namespace("layer_%d" % layer_idx):
        ...
        with flow.scope.namespace("output"):
            ...
            layer_output_blob = _LayerNorm(
                layer_output_blob, hidden_size, trainable=trainable)
            prev_output_blob = layer_output_blob
            all_layer_output_blobs.append(layer_output_blob)
    return prev_output_blob
...
def _FullyConnected(input_blob, input_size, units, activation=None, name=None,
                    weight_initializer=None, trainable=True):
...
def _LayerNorm(input_blob, hidden_size, trainable=True):
    return flow.layers.layer_norm(input_blob, name='LayerNorm', begin_norm_axis=-1, begin_params_axis=-1, trainable=trainable)
...
def _CreateAttentionMaskFromInputMask(to_mask_blob, from_seq_length, to_seq_length):
    ...
    zeros = flow.constant(0.0, dtype=flow.float, shape=[
...
def _EmbeddingPostprocessor(input_blob,
    ...
    if use_position_embeddings:
        position_table = flow.get_variable(name=position_embedding_name,
                                           shape=[
                                               1, max_position_embeddings, embedding_size],
                                           dtype=input_blob.dtype,
                                           initializer=CreateInitializer(initializer_range))
        assert seq_length <= max_position_embeddings
        if seq_length != max_position_embeddings:
            position_table = flow.slice(position_table, begin=[
                                        None, 0, 0], size=[None, seq_length, -1])
...
def _EmbeddingLookup(input_ids_blob,
    ...
    embedding_table = flow.get_variable(name=word_embedding_name, shape=[vocab_size, embedding_size],
                                        dtype=flow.float,
                                        initializer=CreateInitializer(initializer_range))
    output = flow.gather(params=embedding_table,
                         indices=input_ids_blob, axis=0)
